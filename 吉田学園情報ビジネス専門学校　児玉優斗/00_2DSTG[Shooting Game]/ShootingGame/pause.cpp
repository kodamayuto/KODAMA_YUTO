...
//	マクロ定義
//=============================================================================
#define	PAUSEMENU_NUM_POLYGON (5)
...
void InitPause(void)
{
	...
	D3DXCreateTextureFromFile(pDevice, PAUSE_TEXTURE_005, &g_pTexturePause[4]);
...
void UpdatePause(void)
{
	...
	{
		g_nSerectPauseMenu = (g_nSerectPauseMenu + 2) % 3;//0→2→1→0
		PlaySound(SOUND_LABEL_SE_SELECT);
	}
	else if (GetKeyboardTrigger(DIK_S) == true && g_bPauseMenuDisp == true)//↓
	{
		g_nSerectPauseMenu = (g_nSerectPauseMenu + 1) % 3;//0→1→2→0
		PlaySound(SOUND_LABEL_SE_SELECT);
	}
...
void MakeVertexPause(LPDIRECT3DDEVICE9 pDevice)
{
	...
	/*ポーズUI*/
	for (int nCntPausePolygon = 0; nCntPausePolygon < 3; nCntPausePolygon++)
	{

		pVtx[0].pos = D3DXVECTOR3(50, 140.0f + (150 * nCntPausePolygon), 0.0f);
		pVtx[1].pos = D3DXVECTOR3(480, 140.0f + (150 * nCntPausePolygon), 0.0f);
		pVtx[2].pos = D3DXVECTOR3(50, 190.0f + (150 * nCntPausePolygon), 0.0f);
		pVtx[3].pos = D3DXVECTOR3(480, 190.0f + (150 * nCntPausePolygon), 0.0f);
