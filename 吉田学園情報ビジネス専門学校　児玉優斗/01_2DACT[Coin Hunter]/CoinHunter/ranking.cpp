...
void UpdateRanking(void)
{
 ...
 //ENTERキーが押されたかどうか
	if (GetKeyboardTrigger(DIK_RETURN) == true || g_nCntTimer % 800 == 0)
	{
		if (*pFade != FADE_OUT)
		{
			SetFade(MODE_TITLE);
	...
	pVtx[0].tex = D3DXVECTOR2(0.00004f * -g_nCntTimer + 0.0f,0.0f);
	pVtx[1].tex = D3DXVECTOR2(0.00004f * -g_nCntTimer + 1.0f,0.0f);
	pVtx[2].tex = D3DXVECTOR2(0.00004f * -g_nCntTimer + 0.0f,1.0f);
	pVtx[3].tex = D3DXVECTOR2(0.00004f * -g_nCntTimer + 1.0f,1.0f);

	pVtx += 4;

	pVtx[0].tex = D3DXVECTOR2(0.00007f * -g_nCntTimer + 0.0f, 0.0f);
	pVtx[1].tex = D3DXVECTOR2(0.00007f * -g_nCntTimer + 1.0f, 0.0f);
	pVtx[2].tex = D3DXVECTOR2(0.00007f * -g_nCntTimer + 0.0f, 1.0f);
	pVtx[3].tex = D3DXVECTOR2(0.00007f * -g_nCntTimer + 1.0f, 1.0f);
...
void MakeVertexRanking(LPDIRECT3DDEVICE9 pDevice)
{
	...
	for (nCntData = 0; nCntData < 2; nCntData++)
	{
		//頂点座標
