...
BYTE g_aKeyState[NUM_KEY_MAX];			// キーボードの入力情報(プレス情報)を取得
BYTE g_aKeyStateTrigger[NUM_KEY_MAX];	// キーボードの入力情報(トリガー情報)を取得
BYTE g_aKeyStateRelease[NUM_KEY_MAX];	// キーボードの入力情報(リリース情報)を取得
...
void UpdateKeyboard(void)
{
	...
	if (SUCCEEDED(g_pDevKeyboard->GetDeviceState(sizeof(aKeyState),&aKeyState[0])))
	{
		for (nCntKey = 0; nCntKey < NUM_KEY_MAX; nCntKey++)
		{
			...
			g_aKeyState[nCntKey] = aKeyState[nCntKey];														 //プレス
		}
	}
	else
	{
