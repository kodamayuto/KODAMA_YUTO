...
//	マクロ定義
//=============================================================================
#define	PAUSEMENU_NUM_POLYGON (3)
...
int g_nPauseCountUp = 0;
int g_nPauseCountDown = 0;
...
void InitPause(void)
{
	...
	D3DXCreateTextureFromFile(pDevice, PAUSE_TEXTURE_003, &g_pTexturePause[1]);
	D3DXCreateTextureFromFile(pDevice, PAUSE_TEXTURE_004, &g_pTexturePause[2]);
...
void UpdatePause(void)
{
	FADE *pFade;
	pFade = GetFade();
	//||KeyState.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_UP

	////ポーズ画面の表示・非表示
	//if (GetKeyboardTrigger(DIK_O) == true ||
	//	KeyState.Gamepad.wButtons & XINPUT_GAMEPAD_BACK)
	//{
	...
	for (int nCntPausePolygon = 1; nCntPausePolygon < PAUSEMENU_NUM_POLYGON; nCntPausePolygon++)
	{
...
void MakeVertexPause(LPDIRECT3DDEVICE9 pDevice)
{
	...
	for (int nCntPausePolygon = 0; nCntPausePolygon < 2; nCntPausePolygon++)
	{
		...
		pVtx[0].pos = D3DXVECTOR3(400, 140.0f + (250 * nCntPausePolygon), 0.0f);
		pVtx[1].pos = D3DXVECTOR3(830, 140.0f + (250 * nCntPausePolygon), 0.0f);
		pVtx[2].pos = D3DXVECTOR3(400, 190.0f + (250 * nCntPausePolygon), 0.0f);
		pVtx[3].pos = D3DXVECTOR3(830, 190.0f + (250 * nCntPausePolygon), 0.0f);
