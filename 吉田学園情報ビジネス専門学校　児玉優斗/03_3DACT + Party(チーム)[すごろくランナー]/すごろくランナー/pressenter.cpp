...
// マクロ定義
//*****************************************************************************
#define TEXTURE_NAME       "data/TEXTURE/pressenter.png"  // 読み込むテクスチャファイル名
...
void InitPressEnter(void)
{
	...
	g_PressEnterCol = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);   // 色を初期化
...
void UpdatePressEnter(void)
{
	...
	if (g_bDisp == true && GetCounterTitle() >= TRANSITION_MODE)
	{// 描画する状態ならば
		...
		{
			if (GetJoyPadTrigger(nCntJoyPad, 0) == TRUE)
			{
				g_PressEnterState = PRESSENTERSTATE_PUSH;  // 押されている状態にする
				PlaySound(SOUND_LABEL_SE000);
			}
