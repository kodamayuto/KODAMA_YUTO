...
// マクロ定義
//*****************************************************************************
#define TEXTURE_NAME       "data/TEXTURE/titlelogo.png" // 読み込むテクスチャファイル名
#define TEXTURE_SIZEX      (600)                           // テクスチャを張り付ける幅
#define TEXTURE_SIZEY      (350)                           // テクスチャを張り付ける高さ
...
int                     g_SetCounter;               // PressEnterをセットするタイミング
bool                    g_bSet;                     // PressEnterをセットするかしないか
													//=============================================================================
													// 初期化処理
													//=============================================================================
void InitTitleLogo(void)
{
	...
	// 値の初期化
	g_PosTitleLogo = D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 - 50, 0.0f);
	g_colorTitleLogo = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	g_MoveTitleLogo = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	g_SetCounter = 0;
	g_bSet = false;
...
{
	if (g_PosTitleLogo.y <= 300.0f)
	{// 真ん中に来るまで
		if (GetKeyboardTrigger(DIK_RETURN) == true)
		{// ENTERキーが押された
			g_PosTitleLogo.y = 300.0f;
		}
		else
		{
	...
	{
		g_SetCounter++;
		if (g_SetCounter > 20 && g_bSet == false)
		{// ある程度時間がたったら
		 //SetPressEnter(D3DXVECTOR3(SCREEN_WIDTH / 2, 600, 0.0f), 270, 60);
			g_bSet = true;
		}
	}
}
