...
void UpdateWall(void)
{
	for (int nCntWall = 0; nCntWall < MAX_WALL; nCntWall++)
	{
		...
		g_Wall[nCntWall].aPos.x = g_Wall[nCntWall].pos.x +
			sinf(g_Wall[nCntWall].rot.y - (D3DX_PI * 0.5f))* g_Wall[nCntWall].fWidth;

		g_Wall[nCntWall].aPos.z = g_Wall[nCntWall].pos.z +
			cosf(g_Wall[nCntWall].rot.y - (D3DX_PI * 0.5f))* g_Wall[nCntWall].fWidth;


		g_Wall[nCntWall].bPos.x = g_Wall[nCntWall].pos.x +
			sinf(g_Wall[nCntWall].rot.y + (D3DX_PI * 0.5f))* g_Wall[nCntWall].fWidth;

		g_Wall[nCntWall].bPos.z = g_Wall[nCntWall].pos.z +
			cosf(g_Wall[nCntWall].rot.y + (D3DX_PI * 0.5f))* g_Wall[nCntWall].fWidth;
...
bool CollisionWall(D3DXVECTOR3 *pPos, D3DXVECTOR3 *pPosOld)
{
	...
	for (int nCntWall = 0; nCntWall < MAX_WALL; nCntWall++)
	{
		if (g_Wall[nCntWall].bUse == true)
		{
			VecA = g_Wall[nCntWall].aPos - g_Wall[nCntWall].bPos;	//壁のベクトル
			VecB = *pPosOld - g_Wall[nCntWall].bPos;					//壁の始点と弾の前座標を結んだベクトル
			VecC = *pPos - g_Wall[nCntWall].bPos;						//壁の始点と弾の現座標を結んだベクトル

			VecD = *pPos - *pPosOld;										//弾のベクトル
			VecE = g_Wall[nCntWall].bPos - *pPosOld;					//弾の前座標と壁の始点を結んだベクトル
			VecF = g_Wall[nCntWall].aPos - *pPosOld;					//弾の前座標と壁の終点を結んだベクトル
